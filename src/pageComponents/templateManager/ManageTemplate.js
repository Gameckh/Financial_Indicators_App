...
import { getAllTemplates, getTemplateByKey, updateTemplate } from "../../requests";
    ...
    initSpread(designer) {
      ...
      getTemplateByKey(this.props.templateKey).then((res) => {
        if (res && res.length > 0) {
          ...
          designer.setData(
            "treeNodeFromJson",
            JSON.stringify(template.ssjson.designerBindingPathSchema)
          );
          designer.setData(
            "oldTreeNodeFromJson",
            JSON.stringify(template.ssjson.designerBindingPathSchema)
          );
          // delete template.ssjson.designerBindingPathSchema
          spread.fromJSON(template.ssjson);
        }
      });
    }
